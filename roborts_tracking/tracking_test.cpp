...
char message2[100];
Rect roi(0,0,0,0);
Rect face_roi(0,0,0,0);
int img_width = 640;//640;
int img_height = 720;//480;
ros::Publisher* detection_pub = NULL;


void face_callback(const vision_msgs::Detection2D& input)
{
  face_roi = cv::Rect(input.bbox.size_x,
                  input.bbox.size_y,
                  input.source_img.width,
                  input.source_img.height);
  cout << "roi:" << face_roi << endl;
  }
...
duration diff;
  ...
  Mat frame = cv_bridge::toCvShare(input,"bgr8")->image;
  ...
  char c = cv::waitKey(10);
    ...
    //Mat frame;
      ...
      switch(tu.getState())
      {
        case TrackingUtility::STATE_IDLE://准备阶段
          roi = tu.getROI();
          sprintf(message2, "Please select ROI and press g");
          break;

        case TrackingUtility::STATE_INIT://开始追踪
          ...
          tracker->init(roi, frame);//初始化追踪
          tu.startTracker();
          break;

        case TrackingUtility::STATE_ONGOING://start tracker
          trackerStartTime  = std::chrono::high_resolution_clock::now();
          roi = tracker->update(frame);//追踪的核心实现方法
          ...
          dx = (int)(roi.x + roi.width/2  - img_width/2);//与中心点的距离x
          dy = (int)(roi.y + roi.height/2 - img_height/2);//与中心点的距离y
          ...
          gimbal_angle.pitch_angle = pitchRate;
          gimbal_angle.yaw_mode = true;
          //gimbal_angle.yaw_angle = yawRate/180.*M_PI/160.*k;
          gimbal_angle.yaw_angle = yawRate;
          detection_pub->publish(gimbal_angle);
      ...
      cv::circle(frame, Point(img_width/2, img_height/2), 5, cv::Scalar(255,0,0), 2, 8);//中心圆
      if(roi.width != 0)
      {
      ...
      cv::rectangle(frame, face_roi, cv::Scalar(0,0,255),1,8,0);
      sprintf(message1,"dx=%04d, dy=%04d",dx, dy);
      putText(frame, message1, Point2f(20,30), FONT_HERSHEY_SIMPLEX, 0.3,  Scalar(0,255,0));
      putText(frame, message2, Point2f(20,60), FONT_HERSHEY_SIMPLEX, 0.3,  Scalar(0,255,0));
      cv::imshow(winName, frame);
      end = std::chrono::high_resolution_clock::now();
      diff = end - start;
      cout << "time" << diff.count() * 1000 << endl;
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber img_sub = private_nh.subscribe("/image_topic", 3, img_callback);
  ros::Subscriber face_sub = private_nh.subscribe("/face_detect",3,face_callback);
  detection_pub = &pub;
